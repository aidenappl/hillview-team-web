			...
			>
				...
				<div className="shrink-0 flex flex-wrap items-center gap-x-3 gap-y-1.5 border-b border-slate-100 bg-slate-50/60 px-5 py-2.5 sm:px-6">
					...
					<span className="text-xs text-slate-500">
						Active {user.last_active ? dayjs(user.last_active).fromNow() : "never"}
					</span>
					{user.updated_at && (
						<>
							<span className="h-3 w-px shrink-0 bg-slate-200" />
							<span className="text-xs text-slate-400">Updated {formatDate(user.updated_at)}</span>
						</>
					)}
					{user.username && (
						<>
					...
					<div className="flex flex-col gap-5">
						...
						<TeamModalInput
							...
							setValue={(value) => {
								...
								if (cleaned !== user.username) inputChange({ username: cleaned });
								else deleteChange("username");
							}}
						/>
						<TeamModalSelect
							title="Account Type"
							...
							setValue={(value) => {
								if (value.id !== user.authentication.id) inputChange({ authentication: value.id });
								else deleteChange("authentication");
							}}
						/>
						<TeamModalInput
...
export default TeamUserInspectionModal;
